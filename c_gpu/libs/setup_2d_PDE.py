...
bottom, left, right: like top, for the bottom, left, and right-side respectively
    ...
    A = sp.sparse.csr_matrix(sp.sparse.coo_matrix((Adat, (Arow, Acol)), dtype=np.float64)) #create COOrdinate sparse A matrix
    b = np.matrix(b, dtype=np.float64).T
...
##        n = A.shape[1]
##        dim = len(A.shape)
##        b = np.matrix(b.toarray());
##        print("For matrix A: m = {} n = {} dim = {}\n".format(m, n, dim))

        psi = np.linalg.solve(A.toarray(),b)
