...
bottom, left, right: like top, for the bottom, left, and right-side respectively
    ...
    size = nx*ny
    #print("# of nodes: {}".format(size))
    Arow = []
    Acol = []
    Adat = []
    #A = coo_matrix(size, size)
    ...
    j = 1
    for i in range(size):
        #this idiom gets repeated a lot; explained here
        Arow.append(i)  #row coord of the data
        Acol.append(i)  #col coord of the data
        Adat.append(-4) #the actual data
        ...
        if j == 1: #apply left boundary for -x
            ...
            if kind == 'dirichlet':
                #set the dirichlet condition in the resultant vector
                b[i] -= value

                
            elif kind == 'neumann':
                #'By default when converting to CSR or CSC format, duplicate (i,j) entries will be summed together.'
                #using the first order approximation because the 2nd-order approximation involves hoping the matrix is big enough
                Arow.append(i)
                Acol.append(i)
                Adat.append(1)
                #A[i,i] += 1
            ...
            if kind == 'dirichlet':
                #set the dirichlet condition in the resultant vector
                b[i] -= value #set the dirichlet condition in the resultant vector
        ...
        if k == 1: #apply bottom boundary for -y.
            ...
            if kind == 'dirichlet':
                ...
                Adat.append(1)
                #A[i,i] += 1 #using the first order approximation because the 2nd-order approximation involves hoping the matrix is big enough 
                b[i] += DY*value #toggle to - if this doens't work
    ...
    Acol = cp.array(Acol)
    Adat = cp.array(Adat)
    A = cp.sparse.csr_matrix(cp.sparse.coo_matrix((Adat, (Arow, Acol)), dtype=cp.float64)) #create CSR sparse A matrix
    
    b = cp.array(b, dtype=cp.float64).T #create vector b
...
using one of three different solvers.
            solver: a string telling us what solver to use. Valid values are cg, ji, and cp.
            ...
            maxiters: The maximum number of iterations for the solver to run. Ignored by ji and cp. Defaults to 1e6'''
    ...
    if solver == 'np':
##        A = cp.matrix(A.toarray());
##        m = A.shape[0]
##        n = A.shape[1]
##        dim = len(A.shape)
##        b = cp.matrix(b.toarray());
##        print("For matrix A: m = {} n = {} dim = {}\n".format(m, n, dim))
        #A, b already sparse
        psi = cp.linalg.solve(A.toarray(),b)
